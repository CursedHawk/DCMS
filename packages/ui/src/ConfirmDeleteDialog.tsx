...
/*
 * Every t() in this package passes a defaultValue.
 *
 * These components run inside whichever app imported them, against that app's i18next
 * instance — so a key the host app has not copied into its own locale files renders as the
 * literal key path. The platform console shipped a confirmation dialog labelled
 * "common.confirmType" with a button reading "actions.cancel" for exactly that reason. A
 * default means the worst case is untranslated English in a dialog that still makes sense.
 */
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        ...
        <div className="space-y-1.5">
          <Label>{t('common.confirmType', { value: confirmationValue, defaultValue: 'Type {{value}} to confirm' })}</Label>
          <Input
            value={typed}
