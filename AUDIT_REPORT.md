...
* **INF-01** (MEDIUM) — **partially FIXED**: the write-enabled `docker-socket-proxy` (CONTAINERS+POST = container create/start = host takeover) is moved off the shared network onto an internal `docker-api` network shared only with site-builder, its sole client (`docker-compose.prod.yml`). Verified by diffing the rendered base+prod+vps config (only those two services change) and a runtime smoke on this box (proxy still reaches the daemon over the socket from an internal network; on-network client gets 200; off-network client cannot connect). Still deferred: NATS `no_auth_user` and Redis without auth — every service's connection string and Vault secrets change together, which needs a staged rollout rather than a single push.
* **PERF-01** (MEDIUM) — **FIXED**: delivery (content-api media + HLS, site-host, admin media) now streams through `ObjectStreaming.WriteObjectAsync` — a HEAD, then a single-range 206/416 or a 200, pulled from MinIO into the response through one pooled buffer; site-host probes candidates by HEAD instead of downloading each. minio-dotnet 7.0.0's `WithOffsetAndLength` turned out unusable (its internal stat rejects MinIO's 206 with `PartialContentException`), so ranges read-and-discard up to the offset — memory is flat, the residual ceiling is internal bandwidth on tail ranges. Still residual (deploy-side): `mem_limit` on public services.
...
* **DEP-01 residual** — **mostly FIXED**: all 13 HIGH admin-only transitive advisories (postcss, nanoid, fast-uri, shell-quote, browserslist, esbuild) are pinned to patched versions within their major via `pnpm-workspace.yaml` overrides. Still residual: `unhead` (fix is 2.x, but `@unhead/vue@1` blocks it), `vitest`/`@vitest/mocker` (fix in vitest 4), `ts-deepmerge` (fix in v8) — each a major bump, all admin-only LOW/MODERATE, left for a dedicated toolchain upgrade.
