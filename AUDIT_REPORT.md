...
| SEC-15 | MEDIUM | Security / Tenancy | RLS policy on `audit.audit_events` does not cover its partitions, which `dcms_rls` is granted on by default | `RlsConfigurator.cs`, `AuditSchemaConfigurator.cs`, `01-rls.sql` | HIGH | VERIFIED (reproduced on postgres:18) |
...
### Deferred — infra / architectural

This set was originally left for a deliberate, separately-verified change. All of it has since
been applied except the SEC-10 residual; each entry below says how it was verified.

* **INF-01** (MEDIUM) — **partially FIXED**: the write-enabled `docker-socket-proxy` (CONTAINERS+POST = container create/start = host takeover) is moved off the shared network onto an internal `docker-api` network shared only with site-builder, its sole client (`docker-compose.prod.yml`). Verified by diffing the rendered base+prod+vps config (only those two services change) and a runtime smoke on this box (proxy still reaches the daemon over the socket from an internal network; on-network client gets 200; off-network client cannot connect). **Now complete**: `no_auth_user` is gone from `nats.conf` and `Nats__Url` carries `app:${NATS_APP_PASSWORD}`; the nats.conf mount moved from the vps overlay into `docker-compose.prod.yml`, because without it a deployed NATS has no authentication at all. Redis gets `--requirepass` and every `ConnectionStrings__Redis` gets `password=` (plus a healthcheck that authenticates — the base file's bare `redis-cli ping` answers NOAUTH and still exits 0 — and the password for Alloy's embedded Redis exporter, whose absence would have rendered the Redis dashboard as "No data"). `scripts/deploy.sh` generates all three secrets into `.env` on a host that has none, so a new host is still a single deploy with no hand-written value. Verified against real containers: nats:2.11 with this config refuses an anonymous publish and accepts `nats://app:pw@…` from the NATS.Net 2.8.1 client the services use; redis:7 refuses an unauthenticated StackExchange.Redis connect and accepts `host:6379,password=…`.
* **PERF-01** (MEDIUM) — **FIXED**: delivery (content-api media + HLS, site-host, admin media) now streams through `ObjectStreaming.WriteObjectAsync` — a HEAD, then a single-range 206/416 or a 200, pulled from MinIO into the response through one pooled buffer; site-host probes candidates by HEAD instead of downloading each. minio-dotnet 7.0.0's `WithOffsetAndLength` turned out unusable (its internal stat rejects MinIO's 206 with `PartialContentException`), so ranges read-and-discard up to the offset — memory is flat, the residual ceiling is internal bandwidth on tail ranges. **Now complete**: `mem_limit` on admin-api (768m), content-api (640m), site-host (512m) and edge (512m), each overridable per host. It is also a correctness change rather than only a guard rail — .NET sizes the GC heap against the cgroup limit, so with none every service sized against the whole 7.6 GB host.
* **SEC-04** (LOW) — **FIXED**: a new internal `edge-trusted` network with an explicitly declared `10.201.0.0/24` carries the edge→Grafana and edge→Forgejo hops, and both header-auth allow-lists name that CIDR (`GF_AUTH_PROXY_WHITELIST`, `FORGEJO__security__REVERSE_PROXY_TRUSTED_PROXIES`, which had been 172.16.0.0/12 — the whole bridge range). Declaring the subnet is what answers `grafana.ini`'s recorded objection that compose assigns addresses dynamically. A new network rather than a subnet pinned onto `default`, which compose would refuse to reconcile on a running host. Grafana and Forgejo stay on `default` too, so each gets an alias that resolves on `edge-trusted` only and the edge addresses them by it — otherwise the plain name resolves to two addresses and the connection picks one. Verified against grafana:12.3.1 on two real networks: the header signs in from 10.201.0.0/24 and is refused 401 `auth-proxy.invalid-ip` from 10.202.0.0/24, while `/login`, `/api/health` and `/metrics` still answer from the non-whitelisted network — so break-glass, the Prometheus scrape and `obs-smoke.sh` are unaffected.
* **SEC-10 residual** (architectural) — **STILL DEFERRED**, and the only one. Moving OIDC tokens out of `localStorage` to a BFF/cookie model is not a patch: the edge already holds a cookie OIDC session for the Grafana/Forgejo routes, so the shape exists, but extending it to `/api/admin/*` means a cookie→bearer transform on every proxied route, reworking `packages/core/auth.ts` (login, silent renew, logout, tenant switching), cookie auth for the SignalR hubs, and — the actual blocker — a real CSRF token scheme. `SameSite=Lax` does not cover this platform: tenant sites on `*.dcms.highgeek.eu` share a registrable domain with `admin.highgeek.eu`, so a tenant-controlled page's POST is *same-site* and carries the cookie. It needs its own design and browser verification. The risk it leaves is the generic one every token-in-JS SPA carries, and the specific path that made it a HIGH — tenant and CDN JavaScript executing in the admin origin via the IDE preview — is closed (iframe origin, SEC-10; CSP, SEC-11).
* **DEP-01 residual** — **FIXED**: the three that needed a major their consumer refused were taken as majors. vitest 3 → 4 in the seven packages that declare it; `@scalar/api-reference-react` 0.7 → 0.9, which ships `@unhead/vue` 2 and has dropped `ts-deepmerge` entirely; `undici` and `@ai-sdk/provider-utils`, which the newer Scalar brought in, patch within their major and joined the override list. `pnpm audit` reports **no known vulnerabilities**, from 37 at the audit. Verified with `pnpm build` (tsc -b + vite, all packages), all 7 vitest suites and `pnpm lint` unchanged.
...
* **ARCH-01** (INFORMATIONAL) — RLS is still not `FORCE`d; EF query filters remain the runtime tenant guard, and moving off that is an ADR-level decision rather than a bug. What *was* wrong is fixed and is recorded separately as **SEC-15**: the backstop was not enforcing what it claimed. `RlsConfigurator` protected `audit.audit_events` but not its monthly partitions, and Postgres checks a query aimed straight at a partition against that partition's own policies while `01-rls.sql`'s `ALTER DEFAULT PRIVILEGES` grants `dcms_rls` SELECT on each new one — so `SELECT * FROM audit.audit_events_2026m09` returned every tenant's audit records to the one role that exists to prove it cannot. Partitions are now protected with their parents and at creation time, and `ApplyAsync` finishes by reading `pg_class.relrowsecurity` and `pg_policy` and throwing if anything it claimed to protect is not — the old "applied to N tenant tables" log line was the length of a hand-written array, printed whether or not the database agreed, past an apply loop that warns and continues. Four regression tests, two of them mutation checks.
