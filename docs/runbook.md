...
- **RLS** — `RlsConfigurator` puts two permissive policies on every tenant-scoped
  ...
  services move onto in ADR 0015 phase 4 — every service that reads tenant data runs as it now (admin-api only in production; in dev it migrates at startup and stays the owner). identity runs as `dcms_identity`. Only the migrate jobs connect as the owner, `dcms`. **A service on
...
### Writing a secret value into Vault by hand, and the stdin trap

Most secrets are generated by `apply.sh --seed`. The ones that come from outside the platform
— an SMTP password, the edge's Cloudflare API token — have to be written by a person, and the
obvious way to do that has a trap that writes an **empty value and reports success**.

`infra/vault/bin/vault` is not the Vault binary. It is a wrapper that runs
`docker exec -i dcms-vault-1 vault`, because no host installs the CLI. The `-i` is what lets
`apply.sh` pipe policies in on stdin — and it means **every** `vault` call in a pipeline
attaches to that stdin and can consume it. So this, which looks careful, does not work:

```sh
# WRONG. The AppRole login eats the payload; the put writes "".
printf '{"Key":"value"}' | ssh vps1 '
  export VAULT_TOKEN=$( . ~/.dcms/vault-ops.env; vault write -field=token auth/approle/login \
      role_id="$VAULT_OPS_ROLE_ID" secret_id="$VAULT_OPS_SECRET_ID" )
  vault kv put -mount=secret dcms/edge -'
```

The login runs first, through the same `docker exec -i`, and drains the JSON meant for the
`put`. Vault then stores a new version containing an empty string. Give every *other* vault
call its own empty stdin:

```sh
printf '{"Edge__Dns__Cloudflare__ApiToken":"%s"}' "$TOKEN" | ssh vps1 '
  cd ~/baas-dcms
  export VAULT_ADDR=http://127.0.0.1:8200
  export PATH="$PWD/infra/vault/bin:$PATH"
  export VAULT_TOKEN=$( . ~/.dcms/vault-ops.env; vault write -field=token auth/approle/login \
      role_id="$VAULT_OPS_ROLE_ID" secret_id="$VAULT_OPS_SECRET_ID" </dev/null )
  vault kv put -mount=secret dcms/edge -'
```

JSON on stdin rather than `Key=value` on the command line keeps the secret out of `ps` and out
of shell history. A full `put` replaces the whole secret, so read the path first and include
every key it already holds — or use `kv patch`, which merges, with the same `</dev/null`.

**Always read it back.** An empty write is worse than no write: `AddDcmsVault` loads Vault
*last*, so an empty value in Vault silently overrides a correct one in the environment. A
length check is enough to catch every failure this section describes.

```sh
vault kv get -format=json secret/dcms/edge </dev/null \
  | python3 -c 'import json,sys; v=json.load(sys.stdin)["data"]["data"]["Edge__Dns__Cloudflare__ApiToken"]; print(len(v), repr(v[:5]))'
```

This is not hypothetical. On 2026-09-07 two consecutive writes of the Cloudflare token stored
an empty string this way, and the version before them held a value that had been pasted wrong
in a way no amount of care at the keyboard would have caught: **93 characters**, a well-formed
53-character `cfat_` token with a well-formed 40-character classic token concatenated onto the
end of it. Cloudflare rejected the *header format* — `6003 / 6111 Invalid format for
Authorization header` — before ever authenticating, which reads like a code fault rather than
a bad paste. Neither half authenticated when split, so it was two dead credentials, not one
live one badly stored.

Cloudflare's own formats are the useful check: an API token is either **53 characters**
(`cfat_`/`cfut_` prefix) or **40** (the classic form). Any other length is a paste error.

```sh
read -rs -p 'token: ' T; echo
case ${#T} in 40|53) ;; *) echo "length ${#T} — not a Cloudflare token; not writing" ;; esac
```
