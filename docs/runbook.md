...
- **RLS** — `RlsConfigurator` puts two permissive policies on every tenant-scoped
  ...
  services move onto in ADR 0015 phase 4 — every service that reads tenant data runs as it now (admin-api only in production; in dev it migrates at startup and stays the owner). **A service on
