...
export function useMedia(folderId?: string) {
  ...
  return useQuery({
     ...
     * finishes — so the grid is correct within a few hundred milliseconds without asking. The
     * 15-second insurance policy that used to sit behind it is gone: it covered a dropped
     * socket, and a dropped socket is now covered by `useHubRevalidation` in the shell, which
     * refetches when the connection returns instead of every fifteen seconds in case it has.
     */
  });
}
...
export function useMediaDetail(id: string | undefined) {
  return useQuery({
    ...
    // Same reasoning as the list above: the hub is the mechanism, and there is no longer an
    // insurance policy in the query because the insurance moved to the shell.
  });
}
