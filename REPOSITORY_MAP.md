...
| DB least privilege | `dcms_sitebuilder` (sites, BYPASSRLS), `dcms_edge` (edge), `dcms_platform` (platform + obs), `dcms_grafana` (obs), `dcms_rls` (read-only test subject), `dcms_app` (NOBYPASSRLS runtime role of admin-api, content-api, site-host, media-worker and ai-gateway) | `infra/postgres/init/02–06*.sh`, `PlatformRoleConfigurator.cs` |
