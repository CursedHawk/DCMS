...
| RLS (defense in depth) | Two permissive policies per tenant table: `tenant_isolation` keyed on `app.tenant_id`, `platform_scope` keyed on `app.scope = 'platform'`. The app still connects as the table owner, so RLS does not yet apply to it; it applies to non-owner roles (`dcms_rls`, and `dcms_app`, which the services move onto in ADR 0015 phase 4). ADR 0005 + ADR 0015. | `Shared.Data/Rls/RlsConfigurator.cs`, `infra/postgres/init/01-rls.sql`, `06-app-role.sh` |
| DB least privilege | `dcms_sitebuilder` (sites, BYPASSRLS), `dcms_edge` (edge), `dcms_platform` (platform + obs), `dcms_grafana` (obs), `dcms_rls` (read-only test subject), `dcms_app` (NOBYPASSRLS runtime role, not yet in use) | `infra/postgres/init/02–06*.sh`, `PlatformRoleConfigurator.cs` |
