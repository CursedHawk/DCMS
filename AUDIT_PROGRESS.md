...
* **In progress (1):** ARCH-01's forced RLS — an ADR-level decision rather than a defect, and now ADR 0015, staged over five deploys with phases 1–3 landed and phase 4 under way (every tenant-reading service moved except admin-api). SEC-10's localStorage→BFF migration is **done** (ADR 0014): designed, staged over five deploys, soaked on vps1 between the cutover and the fallback removal. The soak earned its place — it caught the edge refusing every WebSocket handshake, because a handshake over HTTP/2 is an extended CONNECT rather than a GET.
